    ...
    <div className="flex flex-col items-center px-8 py-16 gap-16">
      ...
      <section className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl w-full">
        {services.map((service) => (
          <GlassCard key={service.title} className="p-8 flex flex-col gap-4">
            ...
            <h3 className="text-white/90 font-semibold text-lg">{service.title}</h3>
            <p className="text-white/60 text-sm leading-relaxed flex-1">{service.desc}</p>
          </GlassCard>
        ))}
      </section>
      ...
      <section className="max-w-5xl w-full">
        <GlassCard className="p-8">
          <h2 className="text-2xl font-semibold text-white/90 mb-6">Pricing Plans</h2>
          <div className="overflow-hidden rounded-2xl bg-black/10">
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white/[0.05] backdrop-blur-xl">
                  <th className="text-left px-6 py-4 text-white/60 font-medium">Plan</th>
                  <th className="text-left px-6 py-4 text-white/60 font-medium">Features</th>
                  <th className="text-left px-6 py-4 text-white/60 font-medium">Timeline</th>
                  <th className="text-left px-6 py-4 text-white/60 font-medium">Price</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={row.plan} className={i < 2 ? "border-b border-white/[0.06]" : ""}>
                    <td className="px-6 py-4 text-white/90 font-medium">{row.plan}</td>
                    <td className="px-6 py-4 text-white/60">{row.features}</td>
                    <td className="px-6 py-4 text-white/60">{row.timeline}</td>
                    <td className="px-6 py-4 text-white/90">{row.price}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <section className="max-w-3xl w-full">
        <GlassCard className="p-10 flex flex-col items-center text-center gap-6">
          <h2 className="text-3xl font-semibold text-white/90">Need a Custom Solution?</h2>
