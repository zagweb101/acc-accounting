    ...
    <div className="flex flex-col items-center px-8 py-16 gap-8" dir="rtl">
      <section className="w-full">
        <GlassCard className="flex flex-col items-center text-center p-10 gap-4">
          <h1 className="text-4xl font-semibold tracking-tight text-gray-900">شجرة الحسابات</h1>
          <p className="text-gray-600">إدارة دليل الحسابات المحاسبي — عرض هرمي مع إضافة وتعديل وحذف ونسخ</p>
        </GlassCard>
      </section>
      ...
      <section className="w-full">
        <GlassCard className="p-6">
          ...
          {loading ? (
            ...
            <p className="text-gray-400 text-center py-12">لا توجد حسابات. أضف حساباً رئيسياً للبدء</p>
          ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <GlassCard className="p-8 w-full max-w-lg mx-4">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">{editTarget ? "تعديل حساب" : parentTarget ? `إضافة حساب فرعي لـ ${parentTarget.name_ar}` : "إضافة حساب رئيسي"}</h2>
            <div className="flex flex-col gap-4">
              {!editTarget && (
                <div className="grid grid-cols-2 gap-4">
                  {!parentTarget && (
                    <div className="flex flex-col gap-2">
                      <label className="text-gray-600 text-sm">الكود</label>
                      <GlassInput value={form.code || ""} onChange={e => setForm({ ...form, code: e.target.value })} placeholder="مثال: 5000" />
                    </div>
                  )}
                  <div className="flex flex-col gap-2">
                    <label className="text-gray-600 text-sm">نوع الحساب</label>
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-gray-600 text-sm">طبيعة الرصيد</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-gray-600 text-sm">الاسم (عربي)</label>
                <GlassInput value={form.name_ar || ""} onChange={e => setForm({ ...form, name_ar: e.target.value })} placeholder="اسم الحساب بالعربية" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-gray-600 text-sm">الاسم (إنجليزي)</label>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <GlassButton onClick={() => { setShowForm(false); setEditTarget(null); setParentTarget(null); setForm(defaultAccount()); }} className="">إلغاء</GlassButton>
              </div>
            </div>
          </GlassCard>
        </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <GlassCard className="p-8 w-full max-w-md mx-4 text-center">
            <p className="text-gray-900 text-lg mb-2">حذف الحساب</p>
              <p className="text-gray-600 mb-6">{`هل أنت متأكد من حذف "${deleteConfirm.name_ar}" (${deleteConfirm.code})؟`}</p>
      ...
      {showCopy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <GlassCard className="p-8 w-full max-w-lg mx-4">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">نسخ شجرة الحسابات</h2>
            <div className="flex flex-col gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-gray-600 text-sm">من نشاط (المصدر)</label>
              ...
              <div className="flex flex-col gap-2">
                <label className="text-gray-600 text-sm">إلى نشاط (الهدف)</label>
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <GlassButton onClick={() => setShowCopy(false)} className="">إلغاء</GlassButton>
              </div>
            </div>
          </GlassCard>
        </div>
