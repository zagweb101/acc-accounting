    ...
    <div className="flex flex-col px-8 py-16 gap-8" dir="rtl">
      <section className="w-full">
        <GlassCard className="flex flex-col items-center text-center p-10 gap-4">
          <h1 className="text-4xl font-semibold tracking-tight text-gray-900">مراكز التكلفة</h1>
          <p className="text-gray-600">إدارة مراكز التكلفة — هيكل هرمي مع تقارير الربحية</p>
        </GlassCard>
      </section>
      ...
      <section className="w-full">
        <GlassCard className="p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">تقرير ربحية مراكز التكلفة</h2>
          <div className="flex items-center gap-4 flex-wrap mb-6">
            <div className="flex flex-col gap-1">
              <label className="text-gray-400 text-xs">من تاريخ</label>
              <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="input-field max-w-[180px]" />
            </div>
            <div className="flex flex-col gap-1">
              <label className="text-gray-400 text-xs">إلى تاريخ</label>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="text-right px-4 py-3 text-gray-600 font-medium">المركز</th>
                    <th className="text-right px-4 py-3 text-gray-600 font-medium">الإيرادات</th>
                    <th className="text-right px-4 py-3 text-gray-600 font-medium">المصروفات</th>
                    <th className="text-right px-4 py-3 text-gray-600 font-medium">الربح / الخسارة</th>
                  </tr>
                </thead>
                <tbody>
          ...
          <GlassCard className="p-8 w-full max-w-md mx-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center gap-3 mt-2">
                ...
                <GlassButton onClick={() => { setShowForm(false); setEditTarget(null); setParentTarget(null); setFormName(""); setFormCode(""); }} className="">إلغاء</GlassButton>
              </div>
            </div>
          </GlassCard>
        </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60">
          <GlassCard className="p-8 w-full max-w-md mx-4 text-center">
            <p className="text-gray-900 text-lg mb-2">حذف مركز تكلفة</p>
            <p className="text-gray-600 mb-6">{`هل أنت متأكد من حذف "${deleteConfirm.name}" (${deleteConfirm.code})؟`}</p>
