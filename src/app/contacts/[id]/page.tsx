...
export default function ContactDetailPage() {
  ...
  const typeColor = contact.type === "customer" ? "bg-emerald-50 text-emerald-700" : contact.type === "supplier" ? "bg-amber-50 text-amber-700" : "bg-blue-50 text-blue-700";
  ...
  return (
    <div className="flex flex-col px-8 py-16 gap-8" dir="rtl">
      ...
      {error && <div className="w-full card px-4 py-3 text-sm text-red-600 border-red-500/20">{error}</div>}
        ...
        <GlassCard className="p-6">
          <div className="flex items-center gap-1 bg-gray-100 rounded-lg p-1 mb-6 w-fit">
            {[
              ...
              <button key={t.k} onClick={() => setTab(t.k)} className={`px-5 py-1.5 rounded-xl text-sm font-medium transition-all ${tab === t.k ? "bg-gray-100 text-gray-900 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>{t.l}</button>
            ))}
          </div>
              ...
              {statement.length === 0 ? (
                ...
                <div className="overflow-x-auto rounded-2xl bg-gray-50">
                    ...
                    <tbody>
                      {statement.map((s, i) => (
                        <tr key={i} className={i < statement.length - 1 ? "border-b border-gray-200" : ""}>
                          ...
                          <td className="px-4 py-3 text-red-600 font-mono text-xs">{s.debit > 0 ? s.debit.toFixed(2) : "—"}</td>
                      ...
                      <tr className="bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-red-600 font-mono font-semibold">{accTotal.debit.toFixed(2)}</td>
                  ...
                  <div className="px-4 py-3 bg-gray-50 text-left border-t border-gray-200">
                    ...
                    <span className={`font-mono font-semibold ${balance >= 0 ? "text-emerald-700" : "text-red-600"}`}>{balance.toFixed(2)}</span>
                  </div>
                </div>
              )}
            </div>
