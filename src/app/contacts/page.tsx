    ...
    <div className="flex flex-col items-center px-8 py-16 gap-8" dir="rtl">
      <section className="max-w-6xl w-full">
        <GlassCard className="flex flex-col items-center text-center p-10 gap-4">
          <h1 className="text-4xl font-semibold tracking-tight text-white/90">جهات الاتصال</h1>
          <p className="text-white/60">إدارة العملاء والموردين — الأرصدة والحدود الائتمانية</p>
        </GlassCard>
      </section>
      ...
      <section className="max-w-6xl w-full">
        <GlassCard className="p-6">
          <div className="flex items-center gap-4 flex-wrap mb-6">
            ...
            <div className="flex items-center gap-1 bg-black/20 rounded-2xl p-1">
              {["all", "customer", "supplier", "both"].map(t => (
                <button key={t} onClick={() => setFilter(t)} className={`px-4 py-1.5 rounded-xl text-sm font-medium transition-all ${filter === t ? "bg-white/10 text-white/90 shadow-sm" : "text-white/40 hover:text-white/70"}`}>
          ...
          {filtered.length === 0 ? (
            <p className="text-white/40 text-center py-12">لا توجد جهات اتصال. أضف واحدة للبدأ</p>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filtered.map(c => {
                ...
                return (
                  <a key={c.id} href={`/contacts/${c.id}`} className="block glass glass-hover p-5 rounded-2xl transition-all no-underline">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <h3 className="text-white/90 font-semibold text-base">{c.name}</h3>
                        <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-xs font-medium ${c.type === "customer" ? "bg-emerald-500/15 text-emerald-300" : c.type === "supplier" ? "bg-amber-500/15 text-amber-300" : "bg-violet-500/15 text-violet-300"}`}>
                          {c.type === "customer" ? "عميل" : c.type === "supplier" ? "مورد" : "عميل ومورد"}
                        </span>
                      </div>
                      <div className="flex gap-1">
                        <button onClick={e => { e.preventDefault(); e.stopPropagation(); openEdit(c); }} className="w-7 h-7 flex items-center justify-center rounded-full bg-white/[0.06] hover:bg-white/[0.12] text-white/60 hover:text-white/90 transition-all text-xs">⚙</button>
                        <button onClick={e => { e.preventDefault(); e.stopPropagation(); setDeleteConfirm(c); }} className="w-7 h-7 flex items-center justify-center rounded-full bg-white/[0.06] hover:bg-red-500/20 text-white/60 hover:text-red-300 transition-all text-xs">✕</button>
                      </div>
                    </div>
                    {c.tax_number && <p className="text-white/40 text-xs font-mono mb-1">ضريبي: {c.tax_number}</p>}
                    {c.phone && <p className="text-white/40 text-xs mb-3">{c.phone}</p>}
                    <div className="flex items-center justify-between pt-3 border-t border-white/[0.06]">
                      <div>
                        <p className="text-white/40 text-xs">المستحق</p>
                        <p className={`font-mono text-sm font-semibold ${overLimit ? "text-red-300" : overdue ? "text-amber-300" : "text-emerald-300"}`}>
                      ...
                      {c.credit_limit > 0 && (
                        <div className="text-left">
                          ...
                          <p className="font-mono text-sm text-white/70">{c.credit_limit.toFixed(2)}</p>
                        </div>
                      )}
                    </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <GlassCard className="p-8 w-full max-w-lg mx-4">
            <h2 className="text-xl font-semibold text-white/90 mb-6">{editTarget ? `تعديل ${editTarget.name}` : "إضافة جهة اتصال"}</h2>
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select value={formType} onChange={e => setFormType(e.target.value)} className="glass-input cursor-pointer">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-white/60 text-sm">الرقم الضريبي</label>
                  <GlassInput value={formTax} onChange={e => setFormTax(e.target.value)} placeholder="123456789" />
                </div>
                <div className="flex flex-col gap-2">
                  <label className="text-white/60 text-sm">رقم الجوال</label>
                  <GlassInput value={formPhone} onChange={e => setFormPhone(e.target.value)} placeholder="0555123456" />
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-white/60 text-sm">الحد الائتماني</label>
                <input type="number" step="0.01" min="0" value={formCredit} onChange={e => setFormCredit(parseFloat(e.target.value) || 0)} className="glass-input" />
              </div>
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <GlassCard className="p-8 w-full max-w-md mx-4 text-center">
            <p className="text-white/90 text-lg mb-2">حذف جهة اتصال</p>
